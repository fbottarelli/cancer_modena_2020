...
```{r}
# 12 month
bn_covid = glm.nb(df_v$count~df_v$covid+dm[,8]) # negative binomial regression model
summary(bn_covid)
```
...
```{r}
# percentage difference after covid can be calculated with
...
bn_CI = c( (1-exp(confint(bn_covid)[2,1]))*100 , (1-exp(confint(bn_covid)[2,2]))*100); bn_CI
```
...
```{r}
# comparison between poisson and negative binomial regression with log-likelihood, possible because Negative binomial can be viewed like a more generalized version of the Poisson.
poi_covid <- glm(df_v$count ~ df_v$covid, family = "poisson")
pchisq(2 * (logLik(bn_covid) - logLik(poi_covid)), df = 1, lower.tail = FALSE)
```
the p-value $\approx 0$ confirm the correct assumption of negative binomial instead of Poisson. (presence of overdispersion in the data)
...
total_c=NUMBER_CANCERS_ITALY_2020_F+NUMBER_CANCERS_ITALY_2020_M

lost_cases = (total_c/100*perce);lost_cases

# confidence interval prediction
c( (total_c/100*bn_CI[1]), (total_c/100*bn_CI[2]))
